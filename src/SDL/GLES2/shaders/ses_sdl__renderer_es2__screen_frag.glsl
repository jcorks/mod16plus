...
bool is32shadel(mediump float a, mediump float b) {
    return int(a * 64.0) < int(b * 64.0);
}

bool is64shadel(mediump float a, mediump float b) {
...
void main() {  
    ...
    mediump vec4 shadeReal = texture2D(sampler, uv_frag);

    /// R5 G6 R5
    mediump vec4 shade16 = vec4(
